...
u8 mpu6050_ok;
void ST_MPU6050_Read(void)
{
    IIC_FastMode = 1;
    ST_IIC_Read_nByte(MPU6050_ADDR,MPU6050_RA_ACCEL_XOUT_H,14,mpu6050_buffer);
}
...
失败为0
*******************************************************************************/
void ST_IICwriteBit(u8 dev, u8 reg, u8 bitNum, u8 data) {
    u8 b;
    ST_IIC_Read_nByte(dev, reg, 1, &b);
    b = (data != 0) ? (b | (1 << bitNum)) : (b & ~(1 << bitNum));
    mpu6050_ok = !( ST_IIC_Write_1Byte(dev, reg, b) );
}
/**************************实现函数********************************************
...
失败为0
*******************************************************************************/
void ST_IICwriteBits(u8 dev,u8 reg,u8 bitStart,u8 length,u8 data)
{
...
*功　　能:	    设置 采样率
*******************************************************************************/
void ST_MPU6050_set_SMPLRT_DIV(uint16_t hz)
{
    ST_IIC_Write_1Byte(MPU6050_ADDR, MPU6050_RA_SMPLRT_DIV,1000/hz - 1);
...
*******************************************************************************/
void ST_MPU6050_setClockSource(uint8_t source)
{
    ST_IICwriteBits(MPU6050_ADDR, MPU6050_RA_PWR_MGMT_1, MPU6050_PWR1_CLKSEL_BIT, MPU6050_PWR1_CLKSEL_LENGTH, source);
...
*/
void ST_MPU6050_setFullScaleGyroRange(uint8_t range) {
    ST_IICwriteBits(MPU6050_ADDR, MPU6050_RA_GYRO_CONFIG, MPU6050_GCONFIG_FS_SEL_BIT, MPU6050_GCONFIG_FS_SEL_LENGTH, range);
    ST_IICwriteBits(MPU6050_ADDR, MPU6050_RA_GYRO_CONFIG,7, 3, 0x00);   //不自检
}
...
*******************************************************************************/
void ST_MPU6050_setFullScaleAccelRange(uint8_t range) {
    ST_IICwriteBits(MPU6050_ADDR, MPU6050_RA_ACCEL_CONFIG, MPU6050_ACONFIG_AFS_SEL_BIT, MPU6050_ACONFIG_AFS_SEL_LENGTH, range);
    ST_IICwriteBits(MPU6050_ADDR, MPU6050_RA_ACCEL_CONFIG,7, 3, 0x00);   //不自检
}
/**************************实现函数********************************************
...
enabled =0   工作
*******************************************************************************/
void ST_MPU6050_setSleepEnabled(uint8_t enabled) {
    ST_IICwriteBit(MPU6050_ADDR, MPU6050_RA_PWR_MGMT_1, MPU6050_PWR1_SLEEP_BIT, enabled);
}
...
*******************************************************************************/
void ST_MPU6050_setI2CMasterModeEnabled(uint8_t enabled) {
    ST_IICwriteBit(MPU6050_ADDR, MPU6050_RA_USER_CTRL, MPU6050_USERCTRL_I2C_MST_EN_BIT, enabled);
}
...
*功　　能:	    设置 MPU6050 是否为AUX I2C线的主机
*******************************************************************************/
void ST_MPU6050_setI2CBypassEnabled(uint8_t enabled) {
    ST_IICwriteBit(MPU6050_ADDR, MPU6050_RA_INT_PIN_CFG, MPU6050_INTCFG_I2C_BYPASS_EN_BIT, enabled);
}
/**************************实现函数********************************************
*函数原型:
*功　　能:	    设置低通滤波截止频率
*******************************************************************************/
void ST_MPU6050_setDLPF(uint8_t mode)
{
    ST_IICwriteBits(MPU6050_ADDR, MPU6050_RA_CONFIG, MPU6050_CFG_DLPF_CFG_BIT, MPU6050_CFG_DLPF_CFG_LENGTH, mode);
}

void ST_MPU6050_INT_Config()
{
...
*******************************************************************************/
void ST_MPU6050_Init(u16 lpf)
{
    u16 default_filter = 1;

    ST_MPU6050_INT_Config();
...
//	IIC_Write_1Byte(MPU6050_ADDR,MPU6050_RA_PWR_MGMT_1, 0x80);
    ...
    ST_MPU6050_setSleepEnabled(0); //进入工作状态
    ST_Delay_ms(10);
    ST_MPU6050_setClockSource(MPU6050_CLOCK_PLL_ZGYRO); //设置时钟  0x6b   0x03
    ST_Delay_ms(10);
    ST_MPU6050_set_SMPLRT_DIV(1000);  //1000hz
    ST_Delay_ms(10);
    ST_MPU6050_setFullScaleGyroRange(MPU6050_GYRO_FS_2000);//陀螺仪最大量程 +-2000度每秒
    ST_Delay_ms(10);
    ST_MPU6050_setFullScaleAccelRange(MPU6050_ACCEL_FS_8);	//加速度度最大量程 +-8G
    ST_Delay_ms(10);
    ST_MPU6050_setDLPF(default_filter);  //42hz
    ST_Delay_ms(10);
    ST_MPU6050_setI2CMasterModeEnabled(0);	 //不让MPU6050 控制AUXI2C
    ST_Delay_ms(10);
    ST_MPU6050_setI2CBypassEnabled(1);	 //主控制器的I2C与	MPU6050的AUXI2C	直通。控制器可以直接访问HMC5883L
    ST_Delay_ms(10);
}
...
void ST_MPU6050_Data_Prepare(float T)
{
    u8 i;
